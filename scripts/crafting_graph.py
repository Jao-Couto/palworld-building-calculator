...
def build_item_graph(items, recipes, resolve):
    """item_id -> {base, ingredients{resolved_id: qty_per_unit}}.

    The reachable set is anything craftable (a recipe product) plus anything
    used as an ingredient. Ingredient counts are normalized to "quantity
    needed to produce 1 unit". Also returns the list of orphan references
    (item_id, raw_material_id) that didn't resolve to any known item.
    """
    orphans = []
    craftable = {}
    ...
    for product_id, recipe in recipes.items():
        ...
        for i in range(1, RECIPE_MATERIAL_SLOTS + 1):
            ...
            if not resolved:
                ...
                continue
            ingredients[resolved] = material_count / product_count
        craftable[product_id] = ingredients

    reachable = set(craftable)
    for ingredients in craftable.values():
        reachable.update(ingredients)

    graph = {}
    for item_id in sorted(reachable):
        if item_id in craftable:
            graph[item_id] = {"base": False, "ingredients": craftable[item_id]}
        else:
...
def build_building_graph(buildings, resolve):
    ...
    unit. Also returns the list of orphan references (building_id,
    raw_material_id) that didn't resolve to any known item.
    """
    orphans = []
    graph = {}
    ...
    return {
        "_meta": {
            "description": (
                ...
                "Ingredient counts are normalized to 'per 1 unit produced' "
                "for items; building materials are per structure. Referential "
                "and cycle validation passed at generation time. Consumed by "
