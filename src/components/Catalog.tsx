...
import { formatEnumTail, formatQuantity } from '../lib/format';
...
function ItemCard({ entry, db, lang }: { entry: Entry; db: ItemDatabase; lang: Language }) {
  const { item, name } = entry;
  const t = UI_TEXT[lang].catalog;
  const typeLabel = item.category?.typeB ? formatEnumTail(item.category.typeB) : null;
    ...
    <div className="flex flex-col gap-3 rounded-lg border border-slate-700 bg-slate-800/50 p-4">
      ...
      {item.price != null && (
        <div className="flex items-center justify-between rounded-md bg-slate-900 px-3 py-1.5 text-sm">
          ...
          <span className="font-mono text-slate-100">{item.price.toLocaleString()}</span>
        </div>
      )}

      {item.base ? (
        <div className="text-xs uppercase tracking-wide text-slate-500">{t.rawMaterial}</div>
      ) : (
          ...
          <div>
            ...
            <ul className="space-y-1 text-sm">
              {ingredientEntries.map(([ingredientId, quantity]) => (
                <li key={ingredientId} className="flex items-center justify-between">
                  <span className="text-slate-300">
                    {resolveName(db[ingredientId]?.name, lang, ingredientId)}
                  </span>
                  <span className="font-mono text-slate-400">{formatQuantity(quantity)}</span>
                </li>
              ))}
            </ul>
          </div>
...
export function Catalog({ db, lang }: CatalogProps) {
  ...
  const typeOptions = useMemo(() => {
    ...
    for (const { item } of entries) {
      if (item.category?.typeB) values.add(item.category.typeB);
    }
  ...
  const filtered = useMemo(() => {
    ...
    return entries.filter(({ id, item, name }) => {
      ...
      if (typeFilter !== ALL && item.category?.typeB !== typeFilter) return false;
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 rounded-lg border border-slate-700 bg-slate-800/50 p-4 sm:grid-cols-2 lg:grid-cols-4">
        <div>
        ...
        <div>
          ...
          >
            ...
            {typeOptions.map((typeB) => (
              <option key={typeB} value={typeB}>
                {formatEnumTail(typeB)}
              </option>
            ))}
          </select>
        </div>
